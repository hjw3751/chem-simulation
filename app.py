...
# ==========================================
# 2. RK4(Runge-Kutta 4th Order) 수치해석 엔진
# ==========================================
...
# ==========================================
# 3. 메인 화면 UI 및 사용법 가이드
# ==========================================
st.title("🧪 RK4 수치해석 기반 화학 평형 & 르샤틀리에 시뮬레이터")
st.caption(
    "컴퓨터공학 X 화학 II 융합 탐구: 라이브러리 없이 직접 구현한 4차 룽게-쿠타(RK4) 알고리즘"
)
...
with st.expander("📖 처음 오셨나요? 시뮬레이터 사용법 & 화학 개념 설명", expanded=True):
    st.markdown("""
    ### 💡 이 시뮬레이터는 무엇인가요?
    가역 반응 $A + B \rightleftharpoons C + D$ 에서 조건이 변할 때 **동적 평형 상태**와 **르샤틀리에 원리**에 의해 반응이 어느 방향으로 이동하는지 **수치해석 알고리즘(RK4)**으로 실시간 계산하여 시각화하는 웹 도구입니다.

    ---
    ### ⚙️ 어떻게 사용하나요?
    1. **왼쪽 사이드바**에서 정반응/역반응 속도상수와 초기 농도를 조절해 보세요.
    2. **20초 시점 자극 설정**에서 반응물 $A$나 생성물 $C$의 농도를 늘리거나 줄여보세요.
...
if Q_pert < K_eq - 1e-4:
    direction_text = "정반응 진행 (Q < K)"
    status_color = "blue"
elif Q_pert > K_eq + 1e-4:
    direction_text = "역반응 진행 (Q > K)"
    status_color = "red"
else:
    direction_text = "평형 유지 (Q ≈ K)"
    status_color = "green"
...
labels = ["반응물 [A]", "반응물 [B]", "생성물 [C]", "생성물 [D]"]
...
ax.set_title(
    f"RK4 수치해석 시뮬레이션 결과 : [{direction_text}]", fontsize=14, pad=10
)
ax.set_xlabel("시간 (초)", fontsize=12)
ax.set_ylabel("농도 (M)", fontsize=12)
